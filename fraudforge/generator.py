...
class TransactionGenerator:
    ...
    def _build_writer(self, cfg: GeneratorConfig) -> Writer:
        ...
        writer_kwargs: dict[str, BucketExporter] = {}
        if bucket is not None:
            writer_kwargs["bucket"] = bucket
        if cfg.output.format == "csv":
            return CSVWriter(outdir, **writer_kwargs)
        if cfg.output.format == "json":
            return JSONWriter(outdir, **writer_kwargs)
        if cfg.output.format == "parquet":
            return ParquetWriter(outdir, **writer_kwargs)
